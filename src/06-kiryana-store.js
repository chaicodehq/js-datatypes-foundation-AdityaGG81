...
export function calculateTotal(items) {
  ...
  return items.reduce((sum, currentItem) => sum + (currentItem.price * currentItem.qty), 0)
}

export function sortByPrice(items, ascending) {
  if(!Array.isArray(items)) return []
  const newItems = [...items]
  return ascending ? newItems.sort((a, b) => a.price - b.price) : newItems.sort((a, b) => b.price - a.price)
}

export function formatBill(items) {
  if(!Array.isArray(items) || items.length === 0) return ""
  const billArr = items.map((item) => `${item.name} x ${item.qty} = Rs.${item.price * item.qty}`)
  return billArr.join("\n")
}
